...
export default function NewSupplierPaymentPage() {
  ...
  const handleSupplierChange = (value: string) => {
    ...
    if (!value) return;
    getPurchaseBalances({ supplier_id: value })
  ...
  const allocatedTotal =
    Object.values(purchaseAllocations).reduce((sum, v) => sum + (Number(v) || 0), 0) +
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const allocations: SupplierPaymentAllocationCreateRequest[] = [
      ...Object.entries(purchaseAllocations)
        .filter(([, v]) => Number(v) > 0)
        .map(([purchaseId, v]) => ({
          allocation_type: "purchase" as const,
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <h1 className="text-2xl font-semibold">Record Supplier Payment</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 rounded bg-white p-6 shadow">
        ...
        {supplierId && (
          <div className="space-y-2 border-t border-gray-100 pt-4">
            <h2 className="text-sm font-semibold">Allocate to outstanding purchases</h2>
            ...
            {!balancesError && balances !== null && balances.length === 0 && (
              <p className="text-sm text-gray-500">No outstanding purchases for this supplier.</p>
            )}
            {!balancesError && balances !== null && balances.length > 0 && (
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-200 text-left text-gray-500">
                    <th className="py-1 pr-2 font-medium">Purchase #</th>
                ...
                <tbody>
                  {balances.map((row) => (
                    <tr key={row.purchase_id} className="border-b border-gray-100 last:border-0">
                      <td className="py-1 pr-2">{row.purchase_number}</td>
                      ...
                      <td className="py-1">
                        <input
                          ...
                          max={row.balance}
                          step="0.01"
                          value={purchaseAllocations[row.purchase_id] ?? ""}
                          onChange={(e) =>
                            setPurchaseAllocations((prev) => ({ ...prev, [row.purchase_id]: e.target.value }))
                          }
        ...
        <div className="flex justify-end border-t border-gray-100 pt-4">
          <button
            type="submit"
            disabled={submitting || !supplierId || !branchId || !amount || remaining !== 0}
