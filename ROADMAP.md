...
1. ✅ Machine Detail/Profitability (computed report, not a stored ledger — equal overhead split across active machines in Stage 1). **Known gap:** revenue attribution is deferred — `InvoiceLineItem` has no FK to `LotColour`/machine, so this stays cost-only until that link exists.
2. ✅ Reporting: Report Library, Financial Reports, Production Reports, Inventory Reports, Machine Reports, Receivable Ageing — shared filter+chart+table layout, PDF/Excel export. **Known gap: Payable Ageing was never built** — no supplier-payment tracking exists in Stage 1, so a true payable balance isn't computable; skipped deliberately, not approximated.
3. ✅ Approvals Centre (single queue across all pending types), Notifications Centre.
4. ✅ Scheduled Report Settings + Celery Beat job that runs weekly/monthly, builds the PDF (WeasyPrint), sends via Resend. Confirmed at the item 10 checkpoint below to be a genuine cron-scheduled send, not a manually-triggered-only endpoint.
...
7. ✅ **Platform Super Admin portal** — separate app area/route group with its own auth boundary: Platform Dashboard, Subscriber Factories (account/subscription metadata only — zero tenant business-data access, no grant/impersonation mechanism), Trial Accounts. **Deferred, not built** (no concrete need yet, same reasoning as Approval Configuration was before Phase 5): Subscription Plans catalog, Platform Billing, Support Requests, System Health & Backups, Platform Settings.
8. ⚠️ Security hardening pass — **mostly done, two open items:**
   - ✅ Rate limiting (Redis-backed, `/auth/login` + `/auth/forgot-password`), fail-loud secrets (startup crashes on default `jwt_secret`/`internal_api_secret` outside local), 2MB request body cap, Dependabot (uv + npm + github-actions).
   - ✅ File-upload validation: moot — no upload endpoint exists anywhere in the app.
   - ✅ Dependency/vuln scan: `pnpm.overrides` patched 10 of 31 npm advisories (glob/lodash/js-yaml/postcss — all transitive/dev-only). `ecdsa` timing-attack (Python) confirmed not exploitable — JWTs are HS256-only.
   - ✅ HTTPS/TLS + encryption-at-rest: satisfied by Vercel/Railway/Neon platform defaults.
   - ✅ Support-access model (§24) — audited end-to-end and **explicitly decided**: zero platform-admin access to tenant business data, no grant mechanism. Enforced going forward by a static regression test (`apps/api/tests/test_platform_admin_boundary.py`), not just true today.
   - ❌ **Encrypted backups + retention policy — real gap, not just documented.** Neon Free plan's PITR/backup retention is **6 hours**. This is a genuine pre-launch blocker for onboarding paying customers with real financial data, not a nice-to-have: upgrading the Neon plan (30-day retention available) or standing up a separate periodic backup/export job needs to happen before Stage 1 can actually launch to paying customers.
   - ⏸️ Next.js 14.2.35 has an open CVE cluster (SSRF, DoS, middleware bypass), fixed only by upgrading to Next 15. Deliberately deferred — needs explicit user go-ahead given its size, not something to restart casually.
9. ✅ Load-test the ledger and stitch-billing calculations — done as property-based (hypothesis) correctness-at-scale testing (`apps/api/tests/test_pricing_properties.py`) against the pure pricing/allocation math, not a DB-backed concurrency/volume harness (none exists in this repo yet). **Found and fixed a real production bug in the process**: the invoice-balance cap check compared money as raw floats, so a customer paying off a balance to the exact cent (e.g. $1.76 + $1.77 against a $3.53 balance) could get spuriously rejected — IEEE754 double addition landed a hair above the total. Fixed by comparing in integer cents.
10. ✅ **Checkpoint / Stage 1 done:** the full pipeline — client registration → lot receiving → production → delivery → invoicing → payment → payroll → reporting → weekly automated emails — is genuinely wired end-to-end with real persistence, real frontend pages, and consistent tenant-isolation (RLS via `require_permission`/`get_current_user`), not stubs. Delivery quantities are actually capped by `min(received, approved production)`, not just UI-suggested. Two named limitations carried forward from items 1–2 above (no Payable Ageing, invoicing not FK-linked to specific deliveries) don't block calling this done — both were deliberately scoped out, not discovered late. The Neon backup-retention gap from item 8 is the one item that should block onboarding real paying customers specifically, separate from Stage 1 feature-completeness.
